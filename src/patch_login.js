...
const path = "c:\\Users\\Asus\\Desktop\\Mental Health\\Mental Health\\MentalHealth\\healthsupport\\src\\pages\\Login.jsx";
...
const stateTarget = `  const [currentQuoteIndex, setCurrentQuoteIndex] = useState(0)
  const captcha = useCaptcha()

  // Inspirational quotes for the left sidebar`;

const stateReplacement = `  const [currentQuoteIndex, setCurrentQuoteIndex] = useState(0)
  const captcha = useCaptcha()
...
const handleSubmitReplacement = `  const handleSubmit = async (e) => {
    e.preventDefault()
    const newErrors = validateForm()
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        if (isSignUp) {
          ...
          await signup({`;
content = content.replace(handleSubmitTarget, handleSubmitReplacement);

const handleSubmitTarget2 = `        } else {
          // Call login function from AuthContext
          const loggedInUser = login(formData.email, formData.password, userRole)
          setSubmitted(true)`;
const handleSubmitReplacement2 = `        } else {
          // Call login function from AuthContext
          const loggedInUser = await login(formData.email, formData.password, userRole)
          setSubmitted(true)`;
content = content.replace(handleSubmitTarget2, handleSubmitReplacement2);

// 4. Forgot password layout wrap
const wrapTarget1 = `                {/* Success Message */}
                {submitted && (`

const wrapReplacement1 = `                {isForgotPassword ? (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-left w-full">
                    ...
                    <button type="button" onClick={() => { setIsForgotPassword(false); setForgotPasswordStep(1); }} className="mt-6 text-sm font-semibold text-slate-500 hover:text-slate-700 w-full text-center">
...
content = content.replace(wrapTarget1, wrapReplacement1);

const wrapTarget2 = `                )}
              </motion.div>`;
const wrapReplacement2 = `                )}
                </>
                )}
              </motion.div>`;

// Replace last occurrence only
const lastIndex = content.lastIndexOf(wrapTarget2);
if (lastIndex !== -1) {
    content = content.substring(0, lastIndex) + wrapReplacement2 + content.substring(lastIndex + wrapTarget2.length);
}

// 5. Forgot password button trigger
const btnTarget = `<a href="#" className="text-primary-600 hover:text-primary-700 font-semibold transition-colors">Forgot Password?</a>`;
const btnReplacement = `<button type="button" onClick={() => { setIsForgotPassword(true); setForgotPasswordStep(1); setErrors({}); }} className="text-primary-600 hover:text-primary-700 font-semibold transition-colors">Forgot Password?</button>`;
